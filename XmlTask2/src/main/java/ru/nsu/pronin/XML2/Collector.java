...
public class Collector {
    ...
    public Map<String, Person> merge() {
        ...
        for (Map.Entry<String, Person> entry : peopleInfo.getIdToPerson().entrySet()) {
            ...
            if (peopleInfo.getNameToPerson().containsKey(fullPerson.getFullName())) {
                peopleInfo.getNameToPerson().get(fullPerson.getFullName()).add(fullPerson); 
            }
        ...
        for (Map.Entry<String, String> relative: person.getRelativeToRole().entrySet()) {
            if (Person.isId(relative.getKey())) {
                if (!idToPerson.containsKey(relative.getKey())) {
                    System.out.println(relative.getKey() + " doesn't exist");
                }
        ...
        for (Map.Entry<String, List<Person>> entry : nameToPerson.entrySet()) {
            ...
            if (ids.size() == 1) {
                ...
                for (Person person: noIds) {
                    mergePersonToId(person, onePerson);
                }
    ...
    public boolean validateRelatives(Person person1, Person person2) {
        if (!Objects.equals(person1.getMother(), null) && (!Objects.equals(person1.getMother(), ""))
            ...
            if (isDifferentPerson(mother1, mother2)) {
                return false;
            }
        }
        if (!Objects.equals(person1.getFather(), null) && !Objects.equals(person1.getFather(), "")
            ...
            if (isDifferentPerson(father1, father2)) {
                return false;
            }
        }
        if (!Objects.equals(person1.getSpouse(), null) && !Objects.equals(person2.getSpouse(), null)) {
            String spouse1 = person1.getSpouse().getName();
            String spouse2 = person2.getSpouse().getName();
            if (isDifferentPerson(spouse1, spouse2)) {
                return false;
            }
        }
        return true;
    ...
    public boolean isDifferentPerson(String name1, String name2) {
    ...
    public static void mergePersonToId(Person pName, Person pId) {
        if (pId.getId() == null) {
