package ru.nsu.pronin.XML1.data;
...
public class Person {
    ...
    public PersonToRole getSpouse() {
        for (Map.Entry<String, String> entry : this.relativeToRole.entrySet()) {
            if (entry.getValue().equals("spouce") || entry.getValue().equals("spouse")
                    || entry.getValue().equals("wife") || entry.getValue().equals("husband")) {
    ...
    public static boolean isSpecificRole(String role) {
        return !(role.equals("spouce") || role.equals("spouse") || role.equals("parent") || role.equals("siblings")
