...
public class Parser {
    ...
    public PeopleInfo readXML(String path) {
        ...
        if (fis == null) {
            throw new IllegalArgumentException("Input XML not found: " + path);
        }
        try {
                    ...
                    switch (element.getName().getLocalPart()) {
                        case "person":
                            ...
                            if (attrId != null) {
                                if (IdToPerson.containsKey(attrId.getValue())) {
                                    Collector.mergePersonToId(IdToPerson.get(attrId.getValue()), person);
                                }
                        ...
                        case "id":
                            ...
                            if (IdToPerson.containsKey(id)) {
                                Collector.mergePersonToId(IdToPerson.get(id), person);
                            }
                            person.setId(id);
                            break;
                        case "fullname":
